...
if(touches.length>0||keyDown("space")){
 ...
 if(ninja.y>700){
  ...
  if(ninja.isTouching(roofgroup)&&ninja.y>=258.5) {
      var token=1;
...
for(x=0;x<coingroup.length;x++){
 ...
 if(distance>=1000){
   fill("black")
   textSize(25)
   text("LEVEL2 COMPLETED",ninja.x+10,canH/2-200)
   text("UNLOCK TREASURE",ninja.x+10,canH/2-130)
 // icon2.visible=true
ninja.velocityX=0;
ninja.velocityY=0;
//console.log(icon2.x)
...
function blackvillan(){
  ...
  if(frameCount%100===0){
    fighter=createSprite(ninja.x+800,canH-400)
